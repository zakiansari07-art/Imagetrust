...
best_val_cp = "gen_model_lr_rate_change_best_validation.pth"

last_cp = "gen_model_lr_rate_change_last_checkpoint.pth"

hist_file_name = "history_lr_rate_change.json"
...
def train_one_epoch(model, device, train_dataloader, criterion, optimizer, epoch):
    ...
    for images, labels, generators in tqdm(train_dataloader, desc=f"Epoch {epoch + 1} Training"):
        images = images.to(device=device, non_blocking=True)
        
        generators = generators.to(device=device, non_blocking=True)
        logits = model(images)

        
        loss = criterion(logits, generators)
...
def validate(model, device, validation_dataloader, criterion, epoch):
    ...
    total = 0
    with torch.no_grad():
        for images, labels, generators in tqdm(validation_dataloader, desc=f"Epoch {epoch + 1} Validation"):
            images = images.to(device=device, non_blocking=True)
...
def main(epochs: int=1):
    ...
    train_dataloader, validation_dataloader = create_generator_dataloader()

    model = create_generator_model()


    checkpoint_name = Path(f"models/checkpoint/{best_val_cp}")
    ...
    history_path = Path(f"src/training/history/{hist_file_name}")
    ...
    for epoch in range(epochs):
        ...
        history["epochs"].append((epoch+1 if len(history["epochs"]) == 0 else history["epochs"][-1]+1))
        ...
        with open(f"src/training/history/{hist_file_name}", "w") as file:
