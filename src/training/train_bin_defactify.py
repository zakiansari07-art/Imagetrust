...
from data.dataloader_defactify import create_binary_task_dataloader
...
best_val_cp = "bin_task_defactify_best_validation.pth"

last_cp = "bin_task_defactify_last_checkpoint.pth"

hist_file_name = "history_bin_task.json"
...
def validate(model, device, validation_dataloader, criterion, epoch):
    ...
    total = 0
    with torch.no_grad():
        for images, labels in tqdm(validation_dataloader, desc=f"Epoch {epoch + 1} Validation"):
            images = images.to(device)
            labels = labels.to(device).float()
...
def main(epochs: int):
    ...
    train_dataloader, validation_dataloader = create_binary_task_dataloader()
    ...
    history_path = Path(f"src/training/bin_task_history/{hist_file_name}")
    ...
    for epoch in range(start_epoch, start_epoch + epochs):
        ...
        with open(f"src/training/bin_task_history/{hist_file_name}", "w") as file:
